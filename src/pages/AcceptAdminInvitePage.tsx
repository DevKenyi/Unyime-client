...
import { Eye, EyeOff, Lock, Home, ArrowLeft } from 'lucide-react'
...
export default function AcceptAdminInvitePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true)
    try {
      const { data } = await api.post<AuthUser>('/api/auth/accept-admin-invite', { token, password })
      login(data)
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 15, color: '#6B7280', marginBottom: 28, textAlign: 'center', maxWidth: 340 }}>
        Choose a password to activate your Unyimi admin account.
      </p>
      ...
      <div style={{ width: '100%', maxWidth: 420, background: '#fff', borderRadius: 20, padding: '28px 28px 24px', boxShadow: '0 4px 24px rgba(0,0,0,0.07)' }}>
        {!token ? (
          <div style={{ textAlign: 'center', padding: '12px 0' }}>
            <p style={{ fontSize: 14.5, color: '#991B1B', margin: '0 0 12px' }}>This invite link is missing its token.</p>
            <Link to="/login" style={{ color: '#095C46', fontWeight: 600, textDecoration: 'none', fontSize: 14 }}>
              Back to sign in
            </Link>
          </div>
        ) : (
